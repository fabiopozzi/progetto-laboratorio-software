...
module Generator
  ...
  def Generator.hook_generator(library_name, functions)
fixed_part =<<'EOS'
...
def Generator.wrapper_generator(libname, library_data)
  # Usa ruby-elf per parsare il binario elf, stampa i simboli globali definiti
  # nella text section del file
  default_path = "../"
  path = default_path + library_name
  wrapper_code = ""
...
EOS
  #library_data is an array containing hashes with informations about functions to be wrapped.
  # Thus it will be something like result = [ { :name => "pippo", :library_path => "/tmp/lib/libpippo.so", :nargs => 4, :args => [int, int, pointer, char_array], :code => "printf("42\n");"} ]
  library_data.each do |function|
    ...
    tmp = function[:nargs] - 1
    tmp.times do
      params_list << " ,void*"
    end

    function_name = function[:name]
    codice = function[:code]
    lib_path = function[:library_path]
...
END_OF_CODE
  ...
  # write the resulting wrapper TODOOO
  #f.puts(include_part)
  #f.puts(wrapper_code)
  #f.close
end
